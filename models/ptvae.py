...
class PtvaeAttentionDecoder(nn.Module):
    ...
    def __init__(self, device=None, note_embedding=None,
                 ...
                 dec_emb_hid_size=128,
                 dec_time_hid_size=1024, dec_notes_hid_size=512,
                 dec_z_in_size=256, dec_dur_hid_size=16, voicing_size=128,
                 attention_emb=32):
        super(PtvaeAttentionDecoder, self).__init__()
        # Parameters
        # note and time
        self.max_pitch = max_pitch  # the highest pitch in train/val set.
        self.min_pitch = min_pitch  # the lowest pitch in train/val set.
    ...
    def decoder(self, z, inference, x, voicing, lengths, teacher_forcing_ratio1,
                teacher_forcing_ratio2):
        # z: (B, z_size)
        # x: (B, num_step, max_simu_note, note_emb_size)
        batch_size = z.size(0)
    ...
    def forward(self, z, inference, x, voicing, lengths, teacher_forcing_ratio1,
                teacher_forcing_ratio2):
        return self.decoder(z, inference, x, voicing, lengths, teacher_forcing_ratio1,
                            teacher_forcing_ratio2)
...
class NoteSummaryAttention(nn.Module):
    ...
    def forward(self, input_q, input_kv):
        q = torch.unsqueeze(self.wq(input_q), dim=-1)
        k = torch.unsqueeze(self.wk(input_kv), dim=-1)
        v = torch.unsqueeze(self.wv(input_kv), dim=-1)
...
if __name__ == '__main__':
    a = torch.zeros(5, dtype=torch.float)
    ind = torch.tensor([1, 1, 3, 4])
    a.scatter(dim=0, index=ind, src=torch.tensor(1, dtype=torch.float))
    print(a)
